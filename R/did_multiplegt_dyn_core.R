...
did_multiplegt_dyn_core <- function(
  ...
  # Ensure df is a data.table (may receive data.frame from polars conversion)
  ...
  for (i in 1:l_u_a_XX) {
    ...
    if (.DID_USE_RCPP()) {
      # Rcpp-optimized lag difference computation
      df[[paste0("diff_y_", i, "_XX")]] <- cpp_lag_diff(df$outcome_XX, df$group_XX, i)
    } else {
    ...
    if (get(paste0("N",increase_XX,"_",i,"_XX")) != 0) {
      ...
      ## Use Rcpp-optimized computation if enabled
      if (.DID_USE_RCPP()) {
        # Rcpp-optimized U_Gg computation
        N_inc <- get(paste0("N",increase_XX,"_",i,"_XX"))
        rcpp_result <- cpp_compute_U_Gg(df, i, G_XX, N_inc, t_min_XX, T_max_XX, increase_XX)
        df[[paste0("U_Gg",i,"_temp_XX")]] <- rcpp_result$U_Gg_temp
        df[[paste0("U_Gg",i,"_XX")]] <- rcpp_result$U_Gg
        df[[paste0("count",i,"_core_XX")]] <- as.numeric(rcpp_result$count_core)
      } else {
        ## Computing U^+_{G,g,l}
      ...
      ## Use Rcpp-optimized variance computation if enabled
      if (.DID_USE_RCPP()) {
        N_inc <- get(paste0("N",increase_XX,"_",i,"_XX"))
        df[[paste0("U_Gg",i,"_temp_var_XX")]] <- cpp_compute_U_Gg_var_temp(df, i, G_XX, N_inc, increase_XX)
      } else {
