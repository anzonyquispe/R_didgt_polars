...
# load("/Users/anzony.quisperojas/Documents/GitHub/R_did_multiplegt_dyn/data/df.RData")
library(data.table)
df <- fread("/Users/anzony.quisperojas/Library/CloudStorage/Dropbox/India_forest_land/A_MicroData/_data_replication.csv")
...
run_did_test <- function() {
  result <- did_multiplegt_dyn(
    df = df,
    outcome = "tree.mcq",
    group = "subdistrict_id",
    time = "year",
    treatment = "post_ror_data_entry",
    effects = 10,
    placebo = 10,
    graph_off = TRUE
  )
